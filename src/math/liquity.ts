...
export function calculateLiquity(
    ...
    invariant(currentSqrtPrice.greaterThanOrEqualTo(lowerSqrtPrice) && currentSqrtPrice.lessThanOrEqualTo(upperSqrtPrice),
        "The current price must in [lowerPrice,upperPrice]")
    let one = new Decimal(1)
    if (direct === 0) {
        let deltaLiquity = desiredAmountSrc.div(one.div(currentSqrtPrice).sub(one.div(upperSqrtPrice)))
        let desiredAmountDst = deltaLiquity.mul(currentSqrtPrice.sub(lowerSqrtPrice))
        return { desiredAmountDst, deltaLiquity }
    } else {
        let deltaLiquity = desiredAmountSrc.div(currentSqrtPrice.sub(lowerSqrtPrice))
        let desiredAmountDst = deltaLiquity.mul(one.div(currentSqrtPrice).sub(one.div(upperSqrtPrice)))
        return { desiredAmountDst, deltaLiquity }
    }
}
