...
import { sqrtPrice2Tick, tick2SqrtPrice, MAX_TICK, MIN_TICK } from "./tick";
...
describe('Tick', function() {
    describe('sqrtPrice2Tick', function() {
        ...
        it('MAX_SQRT_PRICE => MAX_TICK', function() {
            expect(sqrtPrice2Tick(MAX_SQRT_PRICE).toString()).to.equal(MAX_TICK.toString())
        })
        ...
        it('MIN_SQRT_PRICE => MIN_TICK', function() {
            expect(sqrtPrice2Tick(MIN_SQRT_PRICE.toDecimalPlaces(15, Decimal.ROUND_HALF_UP)).toString()).to.equal(MIN_TICK.toString())
        })
    })
    ...
    describe('tick2SqrtPrice', function() {
        ...
        it('MAX_TICK => MAX_SQRT_PRICE', function() {
            expect(tick2SqrtPrice(MAX_TICK).toDecimalPlaces(10).toString()).to.equal(MAX_SQRT_PRICE.toString())
        })
        ...
        it('MIN_TICK => MIN_SQRT_PRICE', function() {
            expect(tick2SqrtPrice(MIN_TICK).toExponential(15).toString()).to.equal(MIN_SQRT_PRICE.toExponential(15).toString())
        })
    })
