...
describe("Tick", function() {
  ...
  describe("tick2SqrtPrice", function() {
    it("20 => 1.001000450120021", function() {
      expect(
        tick2SqrtPrice(20)
          .toDecimalPlaces(15)
          .toString()
      ).to.equal("1.001000450120021");
    });
    it("MAX_TICK => MAX_SQRT_PRICE", function() {
      expect(
        tick2SqrtPrice(MAX_TICK)
          .toDecimalPlaces(10)
          .toString()
      ).to.equal(MAX_SQRT_PRICE.toString());
    });
    ...
    it("MIN_TICK => MIN_SQRT_PRICE", function() {
      expect(
        tick2SqrtPrice(MIN_TICK)
          .toExponential(15)
          .toString()
      ).to.equal(MIN_SQRT_PRICE.toExponential(15).toString());
    });
  });
