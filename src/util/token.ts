import { u64, ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, Token, AccountInfo, AccountLayout } from "@solana/spl-token";
...
export async function getTokenAccount(conn: Connection, address: PublicKey): Promise<AccountInfo> {
    ...
    invariant(account?.data != null, `The token account:${address.toBase58()} data is null`)
 ...
 */
export async function getTokenAccounts(conn: Connection, owner: PublicKey): Promise<AccountInfo[]> {
    let accounts = await conn.getTokenAccountsByOwner(owner, { programId: TOKEN_PROGRAM_ID })
    const accountInfos: AccountInfo[] = []
    for (let i = 0; i < accounts.value.length; i++) {
        let { pubkey, account } = accounts.value[i]
        invariant(account?.data != null, `The token account:${pubkey.toBase58()} data is null`)
        let accountInfo = parseTokenAccount(account)
