...
async function main() {
    ...
    let userTokenA = new PublicKey("5z92F2YZDdVzXuwEVh8FfpCS4LPwo6UyvTuVmiVv32WX")
    let userTokenB = new PublicKey("7RnbD9Nk14ZeT1h3KrfagF45eHaHZespssxdNpWbrd4i")
    ...
    if (maximumAmountA.greaterThan(0)) {
        await swap.approve(userTokenA, swap.tokenSwapInfo.tokenAMint, maximumAmountA, transferAuthority)
    }
    if (maximumAmountB.greaterThan(0)) {
        await swap.approve(userTokenB, swap.tokenSwapInfo.tokenBMint, maximumAmountB, transferAuthority)
    }
    console.log(
        {
